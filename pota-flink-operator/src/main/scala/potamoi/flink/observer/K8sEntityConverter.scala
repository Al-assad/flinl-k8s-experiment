...
import potamoi.flink.share.FlinkOprErr.{IllegalK8sDeploymentEntity, IllegalK8sPodEntity, IllegalK8sServiceEntity}
import potamoi.flink.share.model.{FK8sDeploymentSnap, FK8sPodSnap, FK8sServiceSnap, PodContainerSnap, SvcPort}
...
object K8sEntityConverter {
   ...
   */
  // noinspection DuplicatedCode
  def toPodSnap(pod: Pod): IO[IllegalK8sPodEntity.type, FK8sPodSnap] = ZIO
    .attempt {
      for {
        ...
        conditions = status.conditions
          ...
          .sorted
          .reverse
        phase     = PodPhase.withNameOps(status.phase.getOrElse("")).getOrElse(PodPhase.Unknown)
