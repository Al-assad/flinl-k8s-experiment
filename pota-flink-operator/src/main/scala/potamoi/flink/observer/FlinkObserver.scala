...
object FlinkObserver {
  val live: ZLayer[ActorGuardian with K8sOperator with K8sClient with PotaConf, ActorInteropException, FlinkObserver] = ZLayer {
    for {
      ...
      k8sRefQuery    <- K8sRefQuery.live(potaConf, guardian, k8sClient, k8sOperator)
