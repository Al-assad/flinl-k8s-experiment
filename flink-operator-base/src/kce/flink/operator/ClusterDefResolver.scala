...
import kce.common.safeTrim
import kce.conf.PotaConf
import kce.flink.operator.ClusterDefResolver.notAllowCustomRawConfKeys
import kce.flink.operator.FlinkConfigExtension.{configurationToPF, ConfigurationPF, EmptyConfiguration}
import kce.flink.operator.FlinkOprErr.{DryToFlinkRawConfigErr, ReviseFlinkClusterDefErr}
import kce.flink.operator.FlinkPlugins.{s3Hadoop, s3Presto, s3aPlugins}
import kce.flink.operator.share.{FlinkAppClusterDef, FlinkClusterDef, FlinkSessClusterDef, S3AccessConf}
...
sealed private trait ClusterDefResolver[ClusterDef <: FlinkClusterDef[ClusterDef]] {
  ...
  protected def reviseDefinition(clusterDef: ClusterDef, moreRevisePipe: RevisePipe = identity): IO[ReviseFlinkClusterDefErr, ClusterDef] = {
    val pipe = removeNotAllowCustomRawConfigs andThen
      ...
      moreRevisePipe
    ZIO.attempt(pipe(clusterDef)).mapError(ReviseFlinkClusterDefErr(_))
  }
  ...
  protected def convertToFlinkConfig(
    ...
    ZIO
      ...
      .mapError(DryToFlinkRawConfigErr(_))
  ...
  override def toFlinkRawConfig(clusterDef: FlinkAppClusterDef, potaConf: PotaConf): IO[DryToFlinkRawConfigErr, Configuration] = {
    convertToFlinkConfig(
      ...
      { conf =>
        ...
        val reviseJarPath = if (isS3Path(clusterDef.jobJar)) s"local:///opt/flink/lib/${clusterDef.jobJar.split('/').last}" else clusterDef.jobJar
        conf
